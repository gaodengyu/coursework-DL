...
def svm_loss_naive(
    ...
    for i in range(num_train):
        ...
        for j in range(num_classes):
            ...
            if margin > 0:
                loss += margin
                # SVM梯度：对错误类别j加X[i]，对正确类别y[i]减X[i]
                dW[:, j] += X[i]
                dW[:, y[i]] -= X[i]
...
def svm_loss_vectorized(
    ...
    scores = X.mm(W)
    N = X.shape[0]
    correct_class_scores = scores[torch.arange(N), y].unsqueeze(1)
    margins = (scores - correct_class_scores + 1).clamp(min=0)
    ...
    mask = mask.to(X.dtype)  # 保证和X同类型，避免dtype不一致报错
    dW = X.t().mm(mask) / N
    dW += 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
...
def svm_get_search_params():
    ...
    ###########################################################################
    # SVM参数搜索
    # 扩大搜索范围，增加更多组合，提升找到高准确率模型的概率
    # 经典有效区间，提升准确率和稳定性
    # 扩大搜索范围，尝试更大跨度
    # 优化参数分布，保留表现较好区间并适当扩展，组合数≤25
    learning_rates = [1e-3, 5e-3, 1e-2, 5e-2, 1e-1]
    regularization_strengths = [1e0, 5e0, 1e1, 5e1, 1e2]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def softmax_get_search_params():
    ...
    regularization_strengths = [1e0, 2.5e0, 5e0, 1e1, 5e1]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
